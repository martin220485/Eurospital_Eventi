...
import {
  Calendar, CalendarDays, ChevronDown, Home, LayoutDashboard, LogOut, Menu,
  Ticket, User as UserIcon, X,
} from "lucide-react";
...
const MOBILE_NAV = [
  ...
  { href: "/app/profile", label: "Profilo", icon: UserIcon },
];
    ...
    <>
      <header className="flex h-14 items-center justify-between border-b bg-white px-4 sm:px-6">
        <div className="flex items-center gap-2">
          ...
          <div className="hidden md:block text-sm text-muted-foreground">Area dipendente</div>
        </div>
      ...
      {mobileNav && (
        <div className="fixed inset-0 z-50 md:hidden" onClick={() => setMobileNav(false)}>
          ...
          <aside className="absolute inset-y-0 left-0 flex w-64 flex-col bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
